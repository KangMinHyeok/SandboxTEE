...
int main(int argc, char *argv[]) {
	...
	while (count < 1) {
		...
		n = read(user_client_sockfd, buffer_in, BUFF_LEN);
		if (n < 0) {
		...
		*/
		...
		n = write(adex_sockfd, buffer_in, BUFF_LEN);
		if (n < 0) {
		...
		n = read(adex_sockfd, buffer_in, BUFF_LEN);
		if (n < 0) {
		...
		*/
		...
		n = write(user_client_sockfd, buffer_in, BUFF_LEN);
		if (n < 0) {
