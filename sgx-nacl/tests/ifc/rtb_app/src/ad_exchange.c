...
int main(int argc, char *argv[]) {
	...
	while (count < 1) {
		...
		n = read(ssp_client_sockfd, profile, BUFF_LEN);
		if (n < 0) {
		...
		for (i = 0; i < dsp_num; i++) {
			...
			n = read(dsp_sockfd, buffer, BUFF_LEN);
			if (n < 0) {
				printf("DSP - Read error\n");
				goto exit1;
			}
		...
		n = write(ssp_client_sockfd, buffer, sizeof(bid_list[winner])+sizeof(len)+len+1);
