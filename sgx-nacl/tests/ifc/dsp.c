...
int read_weights(int feat_num) {
	...
	fp = fopen("./rtb_app/data/weight_1458.txt", "r");
	if (fp == NULL) {
	...
	fp = fopen("./rtb_app/data/weight_3358.txt", "r");
	if (fp == NULL) {
		printf("Cannot open weight input\n");
		return -1;
	}
	...
	fp = fopen("./rtb_app/data/weight_3386.txt", "r");
	if (fp == NULL) {
		printf("Cannot open weight input\n");
		return -1;
	}
	...
	fp = fopen("./rtb_app/data/weight_3427.txt", "r");
	if (fp == NULL) {
		printf("Cannot open weight input\n");
		return -1;
	}
	...
	fp = fopen("./rtb_app/data/weight_3476.txt", "r");
	if (fp == NULL) {
		printf("Cannot open weight input\n");
		return -1;
	}
...
int main(int argc, char *argv[]) {
	...
	while (count < 1) {
		...
		n = read_user_data(client_sockfd, buffer_in, BUFF_LEN);
		if (n < 0) {
			printf("Read error\n");
			goto exit1;
		}
		...
		n = send_response(client_sockfd, buffer_out, BUFF_LEN);
		if (n < 0) {
			printf("Write error\n");
			goto exit1;
		}
