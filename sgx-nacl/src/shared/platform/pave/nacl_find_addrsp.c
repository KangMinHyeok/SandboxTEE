...
/*
 * The x86-32 / a architectures use a 32-bit address space, but it is common
...
//  int mmap_type;
  ...
  for (;;) {
    ...
    /* map_addr = mmap((void *) suggested_addr, memory_size,
                    ...
                    -1,
                    0); */
    ocall_mmap_untrusted(-1, 0, memory_size, PROT_NONE, &map_addr);
    /*
    ...
    if (MAP_FAILED == map_addr) {
      ...
      if (-1 == ocall_unmap_untrusted(map_addr, memory_size)) {
        NaClLog(LOG_FATAL,
