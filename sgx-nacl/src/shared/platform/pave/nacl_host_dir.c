...
static ssize_t NaClCopyDirent(struct NaClHostDir *d,
  ...
  int slen = strlen(ldp->d_name);
  if (slen>0 && slen<NACL_ABI_MAXNAMLEN)
    memcpy((char *) nadp->nacl_abi_d_name, ldp->d_name, slen+1);
  ...
  while (len > 0) {
    ...
    if (0 == entry_size) {
      ...
      retval = getdents(d->fd,
                        (struct linux_dirent *) d->dirent_buf,
                        sizeof d->dirent_buf);
      if (-1 == retval) {
        if (xferred > 0) {
...
int NaClHostDirRewind(struct NaClHostDir *d) {
  ...
  return -NaClXlateErrno(ocall_lseek64(d->fd, 0, SEEK_SET));
}
