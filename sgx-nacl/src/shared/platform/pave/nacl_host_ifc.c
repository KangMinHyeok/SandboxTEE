...
#include "native_client/src/shared/platform/nacl_host_ifc.h"
...
struct NaClHostDesc* NaClHostIfcAccept(struct NaClHostDesc *d){
  ...
  rv = ocall_sock_accept(d->d, NULL, NULL, NULL); // NULL, NULL
...
int NaClHostIfcConnect(struct NaClHostDesc *d, struct sockaddr *addr){
  ...
  rv = ocall_sock_connect(AF_INET, SOCK_STREAM, 0, addr, sizeof(struct sockaddr), NULL, NULL, NULL);
