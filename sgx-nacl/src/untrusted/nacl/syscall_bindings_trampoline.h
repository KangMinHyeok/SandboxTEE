...
typedef int (*TYPE_nacl_pipe) (int fd[2]);
typedef int (*TYPE_nacl_fork) (int *pid);

typedef int (*TYPE_nacl_custom) (void);
typedef int (*TYPE_nacl_custom_print) (int prv);

typedef int (*TYPE_nacl_ifc_socket) (int *sockaddr);
typedef int (*TYPE_nacl_ifc_bind) (int s, uint32_t addr, unsigned int addrlen);
typedef int (*TYPE_nacl_ifc_accept) (int s, int adtfd);
typedef int (*TYPE_nacl_ifc_connect) (int s, uint32_t addr, unsigned int len, int adtfd);
typedef int (*TYPE_nacl_ifc_enqueue) (int type, int s, int adtfd);
typedef int (*TYPE_nacl_ifc_dequeue) ();

typedef int (*TYPE_nacl_create_adt) (int *sockaddr, int size);
typedef int (*TYPE_nacl_get_adt_entry) (int adtfd, void *buf, size_t len, int type);
typedef int (*TYPE_nacl_add_adt_entry) (int adtfd, void *buf, size_t len);

typedef int (*TYPE_nacl_receive_adt) (int s, void *buf, size_t len);
typedef int (*TYPE_nacl_propagate_adt) (int s, const void *buf, size_t len);
// typedef int (*TYPE_nacl_sendback_adt) (int s, const void *buf, size_t len);

// typedef int (*TYPE_nacl_snapshot) ();
#if defined(__cplusplus)
}
#endif
