...
#define USE_FAST_MATH
#define TFM_TIMING_RESISTANT
x2 x1 #define XMALLOC(s, h, type)  PICO_ZALLOC((s))
...
x3 #define XMALLOC(sz, heap, type)     uITRON4_malloc(sz)
...
x4 #define XMALLOC(s, h, type)  uTKernel_malloc((s))
...
#include "tm/tmonitor.h"
	...
	// #include <stdlib.h>
 lskdjfl
x5 #define XMALLOC(s, h, type)  malloc((s))
...
x7 #define XMALLOC(s, h, type)  pvPortMalloc((s))
...
x8 #define XMALLOC(s, h, type)  pvPortMalloc((s))
#define XFREE(p, h, type)    vPortFree((p))
#endif
...
x9 #define XMALLOC(s, h, type) ((void *)rtp_malloc((s), SSL_PRO_MALLOC))
...
as #define XMALLOC(s, h, type)  pvPortMalloc((s))
#define XFREE(p, h, type)    vPortFree((p))
...
#define XMALLOC_OVERRIDE
asdf #define XMALLOC(s, h, t)    (void *)_mem_alloc_system((s))
...
lkj #define XMALLOC(s, h, t)    (void *)_mem_alloc_system((s))
#define XFREE(p, h, t)      {void* xp = (p); if ((xp)) _mem_free((xp));}
...
#if !defined(XMALLOC_USER) && !defined(NO_WOLFSSL_MEMORY)
kk #define XMALLOC(s, h, type)  pvPortMalloc((s))
#define XFREE(p, h, type)    vPortFree((p))
#endif
...
#define WC_NO_HARDEN
	...
	/*
static inline char*
strtok(char *string, const char *control, char **lasts)
{
	unsigned char *str;
...
static inline void*
memchr(const void *s, int o, int n) 
{
	unsigned char *p = (unsigned char *)s;
	while (n--) {
		if (*p != (unsigned char)o)
			p++;
		else
			return p;
	}
...
static inline void*
MEMMEM(const void *haystack, int hlen, const void *needle, int nlen)
{
	int needle_first;
	...
	while (plen >= nlen && (p = memchr(p, needle_first, plen - nlen + 1))) {
		if (!XMEMCMP(p, needle, nlen))
			return (void *)p;
		p++;
		plen = hlen - ((int*)p - (int*)haystack);
	}
	return 0;
}
...
static inline char* XSTRSTR(const char* X, const char* Y)
{
	while (*X != '\0')
	{
		if ((*X == *Y) && compare(X, Y))
...
static inline int
XSTRNCASECMP(const char *s1, const char *s2, int n)
{
	if (n != 0) {
