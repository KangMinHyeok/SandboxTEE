...
void NaClLoadSGXSpecific(struct NaClApp *nap) {
	struct NaClDesc *nd;
...
#define SGX_RUNTIME_FILENAME "./scons-out/sgx-linux-x86-64/staging/libsgx_nacl.so"
	...
	nd = (struct NaClDesc *) NaClDescIoDescOpen(SGX_RUNTIME_FILENAME, NACL_ABI_O_RDONLY, 0666);

	if (NULL == nd) {
		NaClLog(LOG_FATAL, "Open SGX_RUNTIME_FILE Error~!\n");
	}

	image = NaClElfImageServiceRuntimeNew(nd, &subret);
	if (image == 0){
		printf("subret= %d\n", subret);
		NaClLog(LOG_FATAL, "Image new error~!\n");
	}

	subret = NaClElfImageLoadServiceRuntime(image, nd, nap);
	if (subret != LOAD_OK) {
		NaClLog(LOG_FATAL, "LOAD_SERVICE_RUNTIME Error~!\n");
	}


#define SSAFRAMENUM 2
#define SSAFRAMESIZE NACL_PAGESIZE
	void *data = malloc (SSAFRAMESIZE*SSAFRAMENUM);
	...
	add_pages_to_enclave(nap->sgx->enclave_secs, stack_addr, 0, 
	...
	struct enclave_tls * gs = data;
	gs->enclave_size = nap->sgx->enclave_secs->size;
	gs->tcs_offset = (uint64_t) tcs_addr;
	gs->initial_stack_offset = ( (unsigned long) stack_addr + 0x100000 ); // TODO ENCLAVE_STACK_SIZE;
	gs->ssa = (void *) ((unsigned long) ssa_addr +	nap->sgx->enclave_secs->baseaddr);
	gs->gpr = (sgx_arch_gpr_t *) ((unsigned long) gs->ssa +
		SSAFRAMESIZE - sizeof(sgx_arch_gpr_t));
	gs->is_main = 1;

	add_pages_to_enclave(nap->sgx->enclave_secs, tls_addr, data, 
			NACL_PAGESIZE, SGX_PAGE_REG, PROT_READ|PROT_WRITE, true, "tls"); 

	memset(data, 0, SSAFRAMESIZE*SSAFRAMENUM);
	sgx_arch_tcs_t * tcs = data;
	tcs->ossa = (uint64_t) ssa_addr;
	tcs->nssa = SSAFRAMENUM;
	// TODO: fix here
	tcs->oentry = 0xc00000000 + NaClElfImageGetEntryPoint(image);
	printf("Oentry %lx\n", tcs->oentry);
	tcs->ofsbasgx = (uint64_t) tls_addr;
	tcs->ogsbasgx = (uint64_t) tls_addr;
	tcs->fslimit = 0xfff;
	tcs->gslimit = 0xfff;	
	add_pages_to_enclave(nap->sgx->enclave_secs, tcs_addr, data, 
			NACL_PAGESIZE, SGX_PAGE_TCS, 0,  true, "tcs"); 
...
NaClErrorCode NaClAppLoadFileAslr(struct NaClDesc *ndp,
	...
	NaClLog(2, "Installing SGX-specific pages\n");
	NaClLoadSGXSpecific(nap);
